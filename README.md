...
The unique solutions are generated by taking each solution in turn and replacing all piece names with _ apart from the two or three pieces that are being considered. The partial solutions are then sorted to together and the ones that only occur once are output as a solution.
